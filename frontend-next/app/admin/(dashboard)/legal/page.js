...
// Ported from frontend/src/pages/AdminLegal.js. Per the migration plan,
// this was moved from pages/AdminLegal.js (a top-level file, not
// pages/admin/AdminLegal.js — no such file exists) to its correct
// app/admin/(dashboard)/legal/ route.
import { useState, useEffect } from 'react';
import { createLegalDocument, updateLegalDocument, publishLegalDocument, getAllLegalDocuments } from '../../../../lib/apiClient';
import { IconClipboard } from '../../../../components/admin/AdminIcons';
...
.al-badge { display: inline-block; padding: 2px 10px; border-radius: 9999px; font-size: 16px; font-weight: 700; text-transform: uppercase; }
.al-badge-pub { background: var(--t05); color: var(--t4); }
.al-badge-draft { background: #fef3c7; color: #d97706; }
.al-btn-edit { padding: 6px 14px; font-size: 16px; font-weight: 700; border-radius: 8px; background: var(--t05); color: var(--t4); border: 1px solid var(--t1); cursor: pointer; }
.al-btn-edit:hover { background: var(--t1); }
.al-btn-pub { padding: 6px 14px; font-size: 16px; font-weight: 700; border-radius: 8px; background: var(--t6); color: #fff; border: none; cursor: pointer; }
...
.al-modal-lbl { display: block; font-size: 16px; font-weight: 700; color: var(--ink); margin-bottom: 6px; }
.al-modal-inp { width: 100%; padding: 12px 16px; border: 1px solid #d1d5db; border-radius: 8px; font-size: 16px; color: var(--ink); background: var(--surface); box-sizing: border-box; }
.al-modal-inp:focus { outline: none; border-color: var(--t4); box-shadow: 0 0 0 3px var(--t05); }
.al-fmt-btn { padding: 4px 10px; font-size: 16px; background: #f3f4f6; border: 1px solid #d1d5db; border-radius: 4px; cursor: pointer; color: var(--ink); }
        ...
        <div className="al-inner">
          <div className="al-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px' }}>
            <div>
              ...
              <p style={{ fontSize: '16px', color: 'rgba(255, 255, 255, 0.75)' }}>Manage your Terms of Service and Privacy Policy</p>
            </div>
            <button onClick={handleCreate} className="al-btn-pub" style={{ padding: '10px 20px', fontSize: '16px' }}>+ New Document</button>
          </div>
          ...
          {success && (
            <div className="al-alert-ok">
              ...
              <p style={{ fontSize: '16px', color: 'var(--t9)' }}>{success}</p>
            </div>
          )}
          ...
          {error && (
            <div className="al-alert-err">
              ...
              <p style={{ fontSize: '16px', color: 'var(--coral)' }}>{error}</p>
            </div>
          )}
          ...
          <div className="al-card">
            <div className="al-card-head">
              ...
              <p style={{ fontSize: '16px', color: 'var(--ink6)', marginTop: '2px' }}>Manage your published and draft documents</p>
            </div>
            ...
            <div className="al-card-body">
              {documents.length === 0 ? (
                <div style={{ padding: '48px 24px', textAlign: 'center' }}>
                  ...
                  <p style={{ fontSize: '16px', fontWeight: 600, color: 'var(--ink)' }}>No legal documents found</p>
                  <p style={{ fontSize: '16px', color: 'var(--ink6)', marginTop: '4px' }}>No legal documents yet. Create your first one to get started.</p>
                  <button onClick={handleCreate} className="al-btn-pub" style={{ marginTop: '16px', padding: '10px 20px', fontSize: '16px' }}>+ Create First Document</button>
                </div>
              ) : (
                documents.map((doc) => {
                  ...
                  return (
                    <div key={doc._id} className="al-doc-row">
                      <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                        ...
                        <div>
                          <h4 style={{ fontSize: '16px', fontWeight: 700, color: 'var(--ink)' }}>{doc.title}</h4>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginTop: '6px' }}>
                            <span className={`al-badge ${doc.status === 'published' ? 'al-badge-pub' : 'al-badge-draft'}`}>{doc.status}</span>
                            <span style={{ fontSize: '16px', color: 'var(--ink6)' }}>v{doc.version}</span>
                            <span style={{ fontSize: '16px', color: 'var(--ink6)' }}>{doc.language}/{doc.region}</span>
                          </div>
                          {doc.summary && <p style={{ fontSize: '16px', color: 'var(--ink6)', marginTop: '6px' }}>{doc.summary}</p>}
                        </div>
                      </div>
              ...
              <div className="al-modal-card">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                  <div>
                    ...
                    <p style={{ fontSize: '16px', color: 'var(--ink6)', marginTop: '4px' }}>{editingDocument ? 'Update your legal document' : 'Add a new legal document'}</p>
                  </div>
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '8px', padding: '10px 12px', background: '#f9fafb', borderRadius: '8px', border: '1px solid #e5e7eb', alignItems: 'center' }}>
                      <span style={{ fontSize: '16px', color: 'var(--ink6)', marginRight: '4px' }}>Format selected text:</span>
                    ...
                    <textarea name="content" value={formData.content} onChange={handleInputChange} onSelect={() => {}} rows={20} className="al-modal-inp" placeholder="Enter your legal document content here..." required style={{ fontFamily: 'monospace', fontSize: '16px' }} />
                    <p style={{ fontSize: '16px', color: 'var(--ink6)', marginTop: '6px' }}>Select text and use buttons above. Use # for large headings, ## for medium, ### for small. Use **bold** and *italic*.</p>
                  </div>
                  ...
                  <div style={{ background: '#f9fafb', borderRadius: '10px', padding: '16px' }}>
                    <label style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', cursor: 'pointer' }}>
                      ...
                      <div>
                        <span style={{ fontSize: '16px', fontWeight: 700, color: 'var(--ink)' }}>Require User Acceptance</span>
                        <p style={{ fontSize: '16px', color: 'var(--ink6)', marginTop: '2px' }}>Users must accept this document before using your service</p>
                      </div>
                    </label>
                  </div>
