...
// Ported from frontend/src/pages/admin/AdminUsers.js, then upgraded for
// Phase 4: the old version fetched every user client-side and filtered in
// JS (matching the old app's behavior, since the backend didn't accept
// page/limit either — see admin.controller.ts's getUsers, now fixed to a
// real aggregation-pipeline pagination). This version sends
// search/role/subscriptionStatus/accountStatus/page/limit to the server
// and renders backend-computed total/pages — same debounced-search +
// numbered-pages pattern as the AdminQuickScans reference implementation.
import { useState, useEffect } from 'react';
import { adminListUsers, adminResetUserUsage, adminUpdateUserSubscription, adminUpdateUserRole, adminUpdateUserStatus, adminToggleInternalFlag } from '../../../../lib/apiClient';
...
.ap-inp:focus { border-color: var(--t4); box-shadow: 0 0 0 2px rgba(1,150,189,0.1); }
...
export default function AdminUsers() {
  ...
  // filterInternal is intentionally excluded — it has no backend query
  // param (see loadUsers below) and is applied client-side over whatever
  // page is already loaded, so changing it shouldn't trigger a refetch.
  useEffect(() => {
    loadUsers();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentPage, pageSize, debouncedSearchQuery, filterRole, filterSubscription, filterAccountStatus]);
  ...
  const loadUsers = async () => {
    try {
      ...
      const response = await adminListUsers({
        ...
        subscriptionStatus: filterSubscription !== 'all' ? filterSubscription : undefined,
      ...
      if (response.error) {
        ...
        // filterInternal has no backend field (isInternal isn't indexed/exposed
        // as a query param) — kept as a client-side filter over this page's
        // rows, same as before, just no longer applied to the full collection.
        setUsers(response.users || []);
        setPagination({
          total: Number(response.total) || 0,
          page: Number(response.page) || currentPage,
          limit: Number(response.limit) || pageSize,
  ...
  const visibleEnd = Math.min(totalRecords, visibleStart + users.length - 1);
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        <div className="ap-card" style={{ padding: '20px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '14px' }}>
            ...
            <div>
              <label className="ap-lbl">Per Page</label>
          ...
          {/* Stats reflect this page's rows only (isInternal/verified aren't
              backend-filterable today) — labeled accordingly so they don't
              read as collection-wide totals now that the table is paginated. */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '12px', marginTop: '20px' }}>
            <div className="ap-stat-mini">
              ...
              <div className="ap-stat-mini-lbl">Real Clients (page)</div>
            </div>
            <div className="ap-stat-mini">
              <div className="ap-stat-mini-val">{users.filter(u => u.isInternal).length}</div>
              <div className="ap-stat-mini-lbl">Internal (page)</div>
            </div>
            <div className="ap-stat-mini">
              <div className="ap-stat-mini-val">{users.filter(u => !u.isInternal && u.subscription?.status === 'active').length}</div>
              <div className="ap-stat-mini-lbl">Active Subs (page)</div>
            </div>
            <div className="ap-stat-mini">
              <div className="ap-stat-mini-val">{users.filter(u => u.role === 'admin').length}</div>
              <div className="ap-stat-mini-lbl">Admins (page)</div>
            </div>
            <div className="ap-stat-mini">
              <div className="ap-stat-mini-val">{totalRecords}</div>
              <div className="ap-stat-mini-lbl">Total Users</div>
            </div>
          </div>
        </div>
        ...
        <div className="ap-card">
          <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--sandd)' }}>
            <span style={{ fontSize: '16px', fontWeight: 600, color: 'var(--ink)' }}>
              Users ({visibleStart}–{visibleEnd} of {totalRecords})
            </span>
          </div>
          ...
          {totalRecords > 0 && (
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '12px', padding: '14px 20px', borderTop: '1px solid var(--sandd)' }}>
              <span style={{ fontSize: '16px', color: 'var(--ink6)' }}>
                Showing {visibleStart}–{visibleEnd} of {totalRecords} users
              </span>
