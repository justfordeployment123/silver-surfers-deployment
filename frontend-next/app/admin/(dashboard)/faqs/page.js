...
// Ported from frontend/src/pages/admin/AdminFAQs.js.
import { useState, useEffect } from 'react';
import { adminListFaqs, adminCreateFaq, adminDeleteFaq, adminUpdateFaq } from '../../../../lib/apiClient';
import { IconCheckCircle, IconPencil, IconTag, IconRuler } from '../../../../components/admin/AdminIcons';
...
.ap-textarea:focus { border-color: var(--t4); box-shadow: 0 0 0 2px rgba(1,150,189,0.1); }
...
.ap-faq-card:hover { box-shadow: 0 4px 16px rgba(16,47,69,0.08); }
    ...
    <>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '12px' }}>
          {[
            { icon: IconCheckCircle, label: 'Published', val: faqs.filter(f => f.published).length },
            { icon: IconPencil, label: 'Drafts', val: faqs.filter(f => !f.published).length },
            { icon: IconTag, label: 'Categories', val: new Set(faqs.map(f => f.category).filter(Boolean)).size },
            { icon: IconRuler, label: 'Avg. Length', val: faqs.length > 0 ? Math.round(faqs.reduce((acc, f) => acc + (f.answer?.length || 0), 0) / faqs.length) : 0 },
