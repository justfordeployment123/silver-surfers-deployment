...
// Ported from frontend/src/pages/Account.js. Wrapped in ProtectedRoute here
// (page-level) rather than inside the component itself, matching the old
// App.js's <Route path="/account" element={<ProtectedRoute><Account />
...
.sp-cmp { background: rgba(1,150,189,0.65); }
...
.ac-card:hover { border-color: rgba(1,150,189,0.4); }
...
.ac-pg-btn { border: 1px solid rgba(255,255,255,0.16); border-radius: 6px; padding: 6px 12px; font-size: 16px; font-weight: 500; color: rgba(255,255,255,0.85); background: rgba(255,255,255,0.05); cursor: pointer; transition: background .15s; }
.ac-pg-btn:hover:not(:disabled) { background: rgba(255,255,255,0.12); }
.ac-pg-btn:disabled { opacity: 0.4; cursor: not-allowed; }
.ac-pg-btn-active { border-color: var(--t4); background: var(--t6); color: #fff; border-radius: 6px; padding: 6px 12px; font-size: 16px; font-weight: 600; cursor: default; }
`;
...
function AccountContent() {
    ...
    // Full audits and quick scans each page independently — switching tabs
    // doesn't reset the other list's page. Search/status stay client-side
    // filters over the current page only (same tradeoff as the admin users
    // table's isInternal filter): the backend paginates by page/limit, not
    // by search text, so a search term narrows what's visible on this page
    // rather than querying the full account history.
    const [analysisPage, setAnalysisPage] = useState(1);
    const [analysisPagination, setAnalysisPagination] = useState({ total: 0, page: 1, limit: 20, pages: 1 });
    const [quickScanPage, setQuickScanPage] = useState(1);
    const [quickScanPagination, setQuickScanPagination] = useState({ total: 0, page: 1, limit: 20, pages: 1 });
    ...
    const load = async () => {
        ...
        const [analysisRes, quickScanRes] = await Promise.all([
            listMyAnalysis({ page: analysisPage, limit: 20 }),
            listMyQuickScans({ page: quickScanPage, limit: 20 }),
        ]);
        ...
        if (analysisRes?.error) { setAuditError(analysisRes.error); setItems([]); }
            ...
            setAnalysisPagination({
                total: Number(analysisRes.total) || 0,
                page: Number(analysisRes.page) || analysisPage,
                limit: Number(analysisRes.limit) || 20,
                pages: Math.max(1, Number(analysisRes.pages) || 1),
            });
        }
        ...
        if (quickScanRes?.error) { setQuickScanError(quickScanRes.error); setQuickScans([]); }
            ...
            setQuickScanPagination({
                total: Number(quickScanRes.total) || 0,
                page: Number(quickScanRes.page) || quickScanPage,
                limit: Number(quickScanRes.limit) || 20,
                pages: Math.max(1, Number(quickScanRes.pages) || 1),
            });
        }
    ...
    }, [user, analysisPage, quickScanPage]);
    ...
    const PaginationBar = ({ pagination, currentPage, setPage }) => {
        if ((pagination.total || 0) <= (pagination.limit || 20)) return null;
        ...
        return (
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '12px', marginTop: '8px' }}>
                <span style={{ fontSize: '16px', color: 'rgba(255,255,255,0.75)' }}>
                    Page {activePage} of {totalPages} ({pagination.total} total)
                </span>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', alignItems: 'center' }}>
                    <button type="button" onClick={() => setPage(1)} disabled={activePage <= 1} className="ac-pg-btn">First</button>
                    <button type="button" onClick={() => setPage(p => Math.max(1, p - 1))} disabled={activePage <= 1} className="ac-pg-btn">Prev</button>
                    {pages.map(p => (
                        <button
                            type="button"
                            key={p}
                            onClick={() => setPage(p)}
                            className={p === activePage ? 'ac-pg-btn-active' : 'ac-pg-btn'}
                        >
                            {p}
                        </button>
                    ))}
                    <button type="button" onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={activePage >= totalPages} className="ac-pg-btn">Next</button>
                    <button type="button" onClick={() => setPage(totalPages)} disabled={activePage >= totalPages} className="ac-pg-btn">Last</button>
                </div>
            </div>
        );
    };
                            ...
                            {subscription && subscription.status === "active" && (
                                <div className="ac-sub-banner">
                                    <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '16px' }}>
                                        ...
                                        <button onClick={() => router.push("/checkout")} className="btn btn-d">
                                            Start Full Audit
                                        </button>
                                    </div>
                                </div>
                            ...
                            <section>
                                <div className="ac-tab-wrap">
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                                        {[
                                            { key: 'full-audits', label: 'Full Audits', desc: 'Detailed audit runs and report packages', count: analysisPagination.total },
                                            { key: 'quick-scans', label: 'Quick Scans', desc: 'Fast scan requests with status progress', count: quickScanPagination.total },
                                ...
                                {activeTab === "full-audits" ? (
                                    <section style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
                                            ...
                                            <span style={{ fontSize: '16px', color: 'rgba(255, 255, 255, 0.75)' }}>{filtered.length} of {items.length} on this page</span>
                                        </div>
                                            ...
                                            <div key={rec._id || rec.taskId} className="ac-card">
                                                <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '16px' }}>
                                                    ...
                                                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                                                        ...
                                                        {rec.taskId && (
                                                            <button onClick={() => router.push(`/account/analysis/${rec.taskId}`)} className="ac-btn-view">
                                        ...
                                        <PaginationBar pagination={analysisPagination} currentPage={analysisPage} setPage={setAnalysisPage} />
                                    </section>
                                ) : (
                                    <section style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
                                            ...
                                            <span style={{ fontSize: '16px', color: 'rgba(255, 255, 255, 0.75)' }}>{filteredQuickScans.length} of {quickScans.length} on this page</span>
                                        </div>
                                            ...
                                            <div key={scan._id || `${scan.email}-${scan.url}-${scan.createdAt || scan.scanDate}`} className="ac-card">
                                                <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-start', justifyContent: 'space-between', gap: '16px' }}>
                                                    ...
                                                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                                                        ...
                                                        {scan._id && (
                                                            <button onClick={() => router.push(`/account/quick-scans/${scan._id}`)} className="ac-btn-view">
                                                                View quick scan
                                                            </button>
                                                        )}
                                                        {scan._id && (
                                        ...
                                        <PaginationBar pagination={quickScanPagination} currentPage={quickScanPage} setPage={setQuickScanPage} />
                                    </section>
                                )}
                            </section>
                        </>
