...
// Ported from frontend/src/pages/AnalysisDetail.js. Dynamic route param
// comes via next/navigation's useParams() (synchronous, no Suspense
// requirement — unlike useSearchParams()) rather than a props.params
// object, since this is a Client Component page.
...
.sb-green { background: rgba(1,150,189,0.2); color: var(--t3); border-color: rgba(1,150,189,0.3); }
...
.wcag-stat-pass { border-radius: 16px; border: 1px solid rgba(1,150,189,0.25); background: rgba(1,150,189,0.12); padding: 20px; }
...
function AnalysisDetailContent() {
  ...
  const sectionTitle = () => ({ fontSize: '22px', fontWeight: 700, color: '#fff' });
  const sectionSub = () => ({ marginTop: '4px', fontSize: '16px', color: 'rgba(255, 255, 255, 0.75)' });
      ...
      <div className="ad-pg">
        <div className="ad-wrap">
          <div style={{ marginBottom: '32px', display: 'flex', flexWrap: 'wrap', gap: '16px', alignItems: 'flex-start', justifyContent: 'space-between' }}>
            <div>
              <button onClick={() => router.push('/account')} className="ad-back">Back to account</button>
