...
// Ported from frontend/src/pages/Login.js. No SSR benefit possible here —
// the whole flow reads/writes localStorage tokens — so this stays fully
// client-rendered, per the migration plan's Group C scope.
//
// Query-param / localStorage-derived initial state (invite token/email,
// redirect target) is read via a typeof-window-guarded lazy useState
// initializer rather than next/navigation's useSearchParams(), which
// forces a <Suspense> boundary. A <Suspense> boundary around a
// useSearchParams-consuming Client Component was found (while porting
// Home) to sometimes leave real SSR'd HTML in place that React never
// hydrates at all (no fiber attached — see components/home/QuickScanSection.js
// for the full writeup). This page instead accepts the far more ordinary,
// well-supported case: the statically-prerendered shell briefly shows
// query-param-independent content before the client-only useEffect below
// fills it in, identical in kind to the theme-init script's already-verified
// data-theme mismatch pattern in app/layout.js.
//
// react-router's `location.state?.from` redirect source is dropped: a
// repo-wide search found nothing ever navigates to /login with a `state`
// object, so it was always undefined in the CRA app too.
import { useEffect, useRef, useState } from 'react';
import { useRouter } from 'next/navigation';
import { API_BASE, fetchJSON } from '../../../lib/publicApi';
import { getMe } from '../../../lib/apiClient';

const getInviteParams = () => {
  if (typeof window === 'undefined') return { inviteToken: null, inviteEmail: null, fromCheckout: false };
  ...
  return {
    ...
    fromCheckout: urlParams.get('from') === 'checkout',
  };
};

const getRedirectTarget = ({ inviteToken, fromCheckout }) => {
  if (inviteToken) return `/team/accept?token=${inviteToken}`;
  ...
  return isValidRoute ? stored : '/';
};

export default function Login() {
  const router = useRouter();
  const [{ inviteToken, inviteEmail, fromCheckout }] = useState(getInviteParams);
  const redirectTo = getRedirectTarget({ inviteToken, fromCheckout });
  ...
  // Pre-fill the invite email once we know it (avoids the SSR/client
  // mismatch of putting it directly in useState's initial value).
  useEffect(() => {
  ...
  async function handleGoogleResponse(resp) {
    ...
    try {
      ...
      try { const payload = JSON.parse(atob(data.token.split('.')[1])); if (payload.role === 'admin') { router.replace('/admin'); return; } } catch {}
      router.replace(redirectTo);
    } catch (e) {
      setError(e.message);
    } finally {
