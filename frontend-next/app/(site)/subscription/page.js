...
// Ported from frontend/src/pages/Subscription.js. Reads ?plan=/?cycle= via
// window.location.search rather than next/navigation's useSearchParams() —
// see app/(site)/login/page.js's comment for the full story on why (a
// <Suspense>-wrapped useSearchParams consumer was found, while porting
// Home, to sometimes never actually hydrate).
...
function SubscriptionContent() {
    ...
    const [{ selectedPlan, selectedCycle }] = useState(() => {
        if (typeof window === 'undefined') return { selectedPlan: null, selectedCycle: 'yearly' };
        const params = new URLSearchParams(window.location.search);
        return {
            selectedPlan: params.get("plan"),
            selectedCycle: params.get("cycle") === "monthly" ? "monthly" : "yearly",
        };
    });
    ...
    const handleSubscribe = async (planId, cycle = "yearly") => {
        try {
                        ...
                        <>
                            ...
                            {currentSubscription.status === "active" && (
                                <div className={`sub-audit-banner${currentSubscription.isTeamMember ? ' team' : ''}`}>
                                    ...
                                    <button onClick={() => router.push("/checkout")} className="btn btn-p">
                                        Start Full Audit
                                    </button>
                                </div>
                            )}
