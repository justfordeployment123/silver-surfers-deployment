...
// Ported from frontend/src/pages/PaymentSuccess.js. Reads ?session_id= via
// window.location.search rather than next/navigation's useSearchParams() —
// see app/(site)/login/page.js's comment for why. The raw fetch call's env
// var changes from REACT_APP_API_BASE_URL to NEXT_PUBLIC_API_BASE_URL,
// matching the rest of the app's consolidated env-var naming.
...
function PaymentSuccessContent() {
  ...
  const confirmOneTimePayment = async (sessionId) => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8000'}/payment-success?session_id=${sessionId}`, {
        ...
        <div className="ps-bg">
          <div className="auth-card" style={{ textAlign: 'center', maxWidth: '460px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              <button onClick={() => router.push('/services')} className="btn btn-o" style={{ flex: 1, justifyContent: 'center' }}>View Plans</button>
              <button onClick={() => router.push('/contact')} className="btn btn-d" style={{ flex: 1, justifyContent: 'center' }}>Contact Support</button>
            </div>
          </div>
        </div>
      </>
      ...
      <div className="ps-bg">
        <div className="ps-card">
          ...
          <div className="ps-actions">
            <button onClick={() => router.push('/checkout')} className="btn btn-p" style={{ flex: 1, justifyContent: 'center' }}>
              Start Your Audit Now
            </button>
            <button onClick={() => router.push('/account')} className="btn btn-o" style={{ flex: 1, justifyContent: 'center' }}>
              View Account
            </button>
          </div>
