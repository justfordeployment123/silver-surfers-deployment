// Ported from frontend/src/pages/FAQ.js. Server Component: fetches FAQs
// server-side (see lib/publicApi.js), eliminating the "Loading FAQs…"
// spinner flash the original always showed on first paint. Only the
// expand/collapse interaction is a client island (components/faq/FaqAccordion.js).
import Link from 'next/link';
import FaqAccordion from '../../../components/faq/FaqAccordion';
import { fetchJSON } from '../../../lib/publicApi';

export const metadata = {
  title: 'FAQ | SilverSurfers',
  description: 'Find answers to the most frequently asked questions about SilverSurfers.',
};

async function getFaqs() {
  const { ok, data } = await fetchJSON('/faqs?published=true', { next: { revalidate: 60 } });
  if (!ok) {
    return { items: [], error: data?.error || 'Failed to load FAQs' };
  }
        ...
        <section className="sec">
          <div className="wrap">
            <div className="faq-list">
              {error ? <p className="faq-status err">{error}</p> : <FaqAccordion items={items} />}
            </div>
          </div>
        </section>
            ...
            <div style={{ maxWidth: '360px', margin: '0 auto' }}>
              <div className="card">
                ...
                <p style={{ fontSize: '16px', color: 'var(--ink6)', lineHeight: '1.65', marginBottom: '16px' }}>
                  See how we&rsquo;ve helped businesses improve their digital platforms
                </p>
                <Link href="/blog" className="btn btn-o">
                  Read Blog
                </Link>
              </div>
            </div>
