...
// Ported from frontend/src/pages/admin/AdminSubscriptionScans.js. Shared by
// four route pages (starter/pro/onetime-scans + the new all-plans
// subscription-scans route) via a planType prop, matching how the old app
// reused this one component across its three thin AdminStarterScans.js /
// AdminProScans.js / AdminOneTimeScans.js wrapper files.
import { useState, useEffect } from 'react';
import { adminListSubscriptionScans } from '../../lib/apiClient';
import { IconStar, IconGem, IconPackage, IconSearch } from './AdminIcons';
...
.ap-btn-green { background: #16a34a; color: #fff; padding: 8px 16px; border-radius: 8px; border: none; cursor: pointer; font-size: 16px; font-weight: 700; display: inline-flex; align-items: center; gap: 6px; transition: background .15s; }
...
const AdminSubscriptionScans = ({ planType = 'all' }) => {
  ...
  useEffect(() => { loadScans(); }, [sortBy, sortOrder, planType]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const SortArrow = ({ field }) => sortBy !== field ? null : <span style={{ fontSize: '16px' }}>{sortOrder === 'asc' ? '↑' : '↓'}</span>;
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        <div className="ap-card">
          <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--sandd)' }}>
            <span style={{ fontSize: '16px', fontWeight: 600, color: 'var(--ink)' }}>Scan Records ({scans.length})</span>
          </div>
          {scans.length > 0 ? (
                ...
                <tbody>
                  {scans.map((scan) => (
                    <tr key={scan._id}>
                      ...
                      <td style={{ whiteSpace: 'nowrap', fontSize: '16px' }}>{new Date(scan.createdAt).toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div style={{ textAlign: 'center', padding: '48px 24px', color: 'var(--ink6)' }}>
              ...
              <p style={{ fontSize: '16px' }}>No scan records found matching your criteria.</p>
            </div>
          )}
        </div>
      </div>
