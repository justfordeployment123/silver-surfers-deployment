...
const CheckItem = ({ children }) => (
  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', color: 'var(--t4)', fontSize: '15px' }}>
    ...
    return (
      <>
        ...
        <div className="ss-bg" style={{ flexDirection: 'column', gap: '12px' }}>
          ...
          <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '15px' }}>Confirming your subscription…</p>
        </div>
      </>
    );
  }
        ...
        <div className="ss-card">
          {success ? (
            <>
              ...
              <p style={{ fontSize: '15px', color: 'var(--ink6)', marginBottom: '28px', lineHeight: '1.6' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                ...
                {!subscriptionLoading && subscription && subscription.isTeamMember && (
                  <div style={{ padding: '12px 16px', background: 'var(--t05)', border: '1px solid var(--t1)', borderRadius: 'var(--r)' }}>
                    <p style={{ fontSize: '13px', color: 'var(--t7)' }}>
                      You're using a team plan. Contact the plan owner to manage the subscription.
                    </p>
                  </div>
                )}
                <button onClick={() => navigate('/')} className="btn btn-o" style={{ width: '100%', justifyContent: 'center' }}>
                  Go to Dashboard
                </button>
              </div>
            </>
            ...
            <>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                ...
                <button onClick={() => navigate('/contact')} className="btn btn-o" style={{ width: '100%', justifyContent: 'center' }}>
                  Contact Support
                </button>
              </div>
            </>
