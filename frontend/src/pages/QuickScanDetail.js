...
.qsd-back { display: inline-flex; align-items: center; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); background: rgba(255,255,255,0.05); padding: 6px 12px; font-size: 12px; color: rgba(255,255,255,0.75); cursor: pointer; margin-bottom: 16px; transition: background .15s; }
...
.qsd-btn-rescan { border-radius: 8px; background: var(--t4); padding: 6px 12px; font-size: 11px; font-weight: 700; color: #fff; border: none; cursor: pointer; transition: background .15s; }
.qsd-btn-rescan:hover { background: var(--t8); }
.qsd-btn-rescan:disabled { opacity: 0.6; cursor: not-allowed; }
.qsd-btn-rerun { border-radius: 8px; background: rgba(217,119,6,0.7); padding: 6px 12px; font-size: 11px; font-weight: 700; color: #fff; border: none; cursor: pointer; }
.qsd-btn-rerun:hover { background: rgba(217,119,6,0.9); }
.qsd-btn-rerun:disabled { opacity: 0.6; cursor: not-allowed; }
.qsd-btn-del { border-radius: 8px; border: 1px solid rgba(248,113,113,0.25); background: rgba(239,68,68,0.08); padding: 6px 12px; font-size: 11px; font-weight: 700; color: #fca5a5; cursor: pointer; }
.qsd-btn-del:hover { background: rgba(239,68,68,0.15); }
.qsd-btn-del:disabled { opacity: 0.6; cursor: not-allowed; }
.qsd-btn-open { border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); background: rgba(255,255,255,0.05); padding: 6px 12px; font-size: 11px; font-weight: 700; color: #fff; cursor: pointer; }
.qsd-btn-open:hover { background: rgba(255,255,255,0.1); }
.qsd-btn-open:disabled { opacity: 0.6; cursor: not-allowed; }
.qsd-btn-dl { border-radius: 8px; background: var(--t4); padding: 6px 12px; font-size: 11px; font-weight: 700; color: #fff; border: none; cursor: pointer; }
        ...
        <>
            ...
            <div className="qsd-pg">
                <div className="qsd-wrap">
                    ...
                    {loading ? (
                        <div style={{ borderRadius: '16px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.18)', padding: '24px', fontSize: '13px', color: 'rgba(255,255,255,0.6)' }}>
                    ...
                    {!loading && !error && item ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
                            {item.emailError ? (
                                <div style={{ borderRadius: '16px', border: '1px solid rgba(251,191,36,0.2)', background: 'rgba(245,158,11,0.1)', padding: '16px', fontSize: '13px', color: '#fcd34d' }}>
                            ...
                            <Section>
                                <div style={{ marginBottom: '20px' }}>
                                    ...
                                    <p style={{ marginTop: '4px', fontSize: '13px', color: 'rgba(255,255,255,0.6)' }}>Open the stored PDF package for this quick scan or download a local copy from your profile.</p>
                                </div>
                                {reportError ? (
                                ...
                                {reportFiles.length === 0 ? (
                                    <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.4)' }}>No report files are available for this quick scan yet.</p>
                                ) : (
                            ...
                            <Section>
                                <div style={{ marginBottom: '20px' }}>
                                    ...
                                    <p style={{ marginTop: '4px', fontSize: '13px', color: 'rgba(255,255,255,0.6)' }}>The weighted Silver Score categories generated for this quick scan.</p>
                                </div>
                                {dimensions.length === 0 ? (
                                    <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.4)' }}>This quick scan does not have a scorecard breakdown yet.</p>
                                ) : (
                            ...
                            <Section>
                                <div style={{ marginBottom: '20px' }}>
                                    ...
                                    <p style={{ marginTop: '4px', fontSize: '13px', color: 'rgba(255,255,255,0.6)' }}>The underlying evaluation dimensions used to calculate the quick scan score.</p>
                                </div>
                                {evaluationDimensions.length === 0 ? (
                                    <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.4)' }}>This quick scan does not include evaluation-dimension data yet.</p>
                                ) : (
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(240px,1fr))', gap: '16px' }}>
                            ...
                            <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(340px,1fr))', gap: '24px' }}>
                                <div style={{ borderRadius: '16px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.2)', padding: '24px' }}>
                                    ...
                                    <p style={{ marginTop: '4px', fontSize: '13px', color: 'rgba(255,255,255,0.6)' }}>Highest-impact findings from this quick scan.</p>
                                    {topIssues.length === 0 ? (
                                ...
                                <div style={{ borderRadius: '16px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.2)', padding: '24px' }}>
                                    ...
                                    <p style={{ marginTop: '4px', fontSize: '13px', color: 'rgba(255,255,255,0.6)' }}>Priority fixes grouped into quick wins, medium effort, and high effort workstreams.</p>
                                                ...
                                                <div key={bucket.key} style={{ borderRadius: '10px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', padding: '16px' }}>
                                                    <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px', marginBottom: '8px' }}>
                                                        <div>
                                                            ...
                                                            <p style={{ marginTop: '6px', fontSize: '13px', color: 'rgba(255,255,255,0.6)' }}>{bucket.description}</p>
                                                        </div>
                                                    </div>
                                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
